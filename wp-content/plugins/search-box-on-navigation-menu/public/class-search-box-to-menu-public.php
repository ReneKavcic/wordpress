...
class Search_On_Menu_Public {
	...
	function search_box_to_menu_script_style(){

		$options = $this->options;

		if ( ! isset( $options['do_not_load_plugin_files']['plugin-css-file'] ) ){
			wp_enqueue_style( 'add-search-to-menu-styles', plugins_url( '/css/search-box-to-menu.css', __FILE__ )  );
		}

		if ( ! isset( $options['do_not_load_plugin_files']['plugin-js-file'] ) && ( isset( $options['search_box_to_menu_style'] ) && $options['search_box_to_menu_style'] != 'default' ) ) {
	...
	function search_box_to_menu_items( $items, $args ){
		...
		if ( isset( $options['search_box_to_menu_locations'] ) ) {
			...
			if ( isset( $options['search_box_to_menu_locations'][$args->theme_location] ) ) {
				...
				if ( $options['search_box_to_menu_style'] != 'default' ){
					if ( $title == '' ) {
