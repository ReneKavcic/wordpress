...
if ( ! class_exists( 'Supermag_Theme_Info' ) ) {
    class Supermag_Theme_Info{
        ...
        public function at_theme_info_welcome_admin_notice() {
            ...
            echo ( '<p>' . sprintf( __('Welcome! Thank you for choosing %1$s! To fully take advantage of the best our theme can offer please make sure you visit our %2$swelcome page%3$s.','dupermag'), $this->theme_name, '<a href="' . esc_url( admin_url( 'themes.php?page=' . $this->theme_slug . '-info' ) ) . '">', '</a>' ) . '</p><p><a href="' . esc_url( admin_url( 'themes.php?page=' . $this->theme_slug . '-info' ) ) . '" class="button" style="text-decoration: none;">' . sprintf( __('Get started with %s','dupermag'), $this->theme_name ) . '</a></p>' );
            echo '</div>';
        }
        ...
        public function prepare_class() {
            ...
            $this->page_title    = isset( $this->config['page_title'] ) ? $this->config['page_title'] : esc_html__('Info','dupermag'). $this->theme_name;
            $this->menu_title    = isset( $this->config['menu_title'] ) ? $this->config['menu_title'] : esc_html__('Info','dupermag') . $this->theme_name;
        ...
        public function recommended_actions() {
            ...
            if ( ! empty( $recommended_actions ) ) {
                        ...
                        if ( ! empty( $plugin_slug ) ) {
                            ...
                            switch ( $active['needs'] ) {
                                ...
                                case 'install':
                                    ...
                                    $label = esc_html__( 'Install', 'dupermag' );
                                    break;

                                case 'activate':
                                    $class = 'activate-now button button-primary';
                                    $label = esc_html__( 'Activate', 'dupermag' );

                                    break;

                                case 'deactivate':
                                    $class = 'deactivate-now button';
                                    $label = esc_html__( 'Deactivate', 'dupermag' );
                ...
                if ( $hooray ){
                    echo '<span class="hooray">' . esc_html__( 'Hooray! There are no recommended actions for you right now.', 'dupermag' ) . '</span>';
                    echo '<a data-action="reset" id="reset" class="reset-all" href="#">'.esc_html__('Show All Recommended Actions','dupermag').'</a>';
                }
                echo '</div>';
            }
        }
        ...
        public function get_plugin_icon( $arr ) {
            ...
            if ( ! empty( $arr['svg'] ) ) {
                ...
                $plugin_icon_url = get_stylesheet_directory_uri() . '/acmethemes/at-theme-info/images/placeholder_plugin.png';
            }
                        ...
                        if( !empty( $screenshot) ){
                            ...
                            if ( ! empty( $title ) ) {
                                ...
                                if ( ! empty( $download_link ) ) {
                                    echo '<a href="' . esc_url( $download_link ) . '" class="button button-primary install right">' . esc_html__( 'Download','dupermag' ) . '</a>';
                                }
                                if ( ! empty( $preview_link ) ) {
                                    echo '<a class="button button-secondary preview right" target="_blank" href="' . $preview_link . '">' . esc_html__( 'Live Preview','dupermag' ). '</a>';
                                }
                                echo '</div>';
                                echo '</div>';
                            }
        ...
        public function free_pro() {
            ...
            if ( ! empty( $free_pro ) ) {
                ...
                if ( ! empty( $free_pro ) && is_array( $free_pro ) ) {
                    ...
                    echo '<tr>';
                    echo '<th></th>';
                    echo '<th>' . esc_html__( 'DuperMag','dupermag' ) . '</th>';
                    echo '<th>' . esc_html__( 'DuperMagPro','dupermag' ) . '</th>';
                    echo '</tr>';
                    echo '</thead>';
                    echo '<tbody>';
                    ...
                    echo '<td></td>';
                    echo '<td colspan="2"><a href="https://www.acmethemes.com/themes/dupermagpro/" target="_blank" class="button button-primary button-hero"> DuperMagPro</a></td>';
                    echo '</tr>';
        ...
        public function style_and_scripts( $hook_suffix ) {
            ...
            wp_enqueue_style( 'at-theme-info-css', get_stylesheet_directory_uri() . '/acmethemes/at-theme-info/css/at-theme-info.css' );

            if ( 'appearance_page_' . $this->theme_slug . '-info' == $hook_suffix ) {

                wp_enqueue_script( 'at-theme-info-js', get_stylesheet_directory_uri() . '/acmethemes/at-theme-info/js/at-theme-info.js', array( 'jquery' ) );
...
$config = array(
    // Page title.
    'page_title'               => esc_html__( 'DuperMag Info', 'dupermag' ),

    // Menu name under Appearance.
    'menu_title'               => esc_html__( 'DuperMag Info', 'dupermag' ),

    // Main welcome title
    'info_title'         => sprintf( esc_html__( 'Welcome to %s - ', 'dupermag' ), 'DuperMag' ),

    // Main welcome content
    'info_content'       => sprintf( esc_html__( '%s is now installed and ready to use. We hope the following information will help you. If you want to ask any query or just want to say hello, you can always contact us. We hope you enjoy it! ', 'dupermag' ), '<b>DuperMag</b>' ),
    ...
    'quick_links'                    => array(
        'theme_url'  => array(
                'text' => __('Theme Details','dupermag'),
                'url' => 'https://www.acmethemes.com/themes/dupermag/'
        ),
        'demo_url'  => array(
            'text' => __('View Demo','dupermag'),
            'url' => 'http://www.demo.acmethemes.com/dupermag/'
        ),
        'pro_url'  => array(
            'text' => __('View Pro Version','dupermag'),
            'url' => 'https://www.acmethemes.com/themes/dupermagpro/'
        ),
        'rate_url'  => array(
            'text' => __('Rate This Theme','dupermag'),
            'url' => 'https://wordpress.org/support/theme/dupermag/reviews/?filter=5'
        ),
    ),

    'tabs'                    => array(
        'getting_started'  => __( 'Getting Started', 'dupermag' ),
        'recommended_actions' => __( 'Recommended Actions', 'dupermag' ),
        'recommended_plugins' => __( 'Useful Plugins','dupermag' ),
        'support'       => __( 'Support', 'dupermag' ),
        'changelog'        => __( 'Changelog', 'dupermag' ),
        'faq'        => __( 'FAQ', 'dupermag' ),
        'free_pro'         => __( 'Free VS PRO', 'dupermag' ),
    ),

    /*Getting started tab*/
    'getting_started' => array(
        'first' => array(
            'title' => esc_html__( 'Step 1 : Read full documentation','dupermag' ),
            'desc' => esc_html__( 'Please check our full documentation for detailed information on how to Setup and Use DuperMag.','dupermag' ),
            'link_title' => esc_html__( 'Documentation','dupermag' ),
            'link_url' => 'http://www.doc.acmethemes.com/dupermag/',
            'is_button' => false,
            'recommended_actions' => false,
            'is_new_tab' => true
        ),
        'second' => array(
            'title' => esc_html__( 'Step 2 : Go to Customizer','dupermag' ),
            'desc' => esc_html__( 'All Setting, Theme Options, Widgets and Menus are available via Customize screen. Have a quick look or start customization!','dupermag' ),
            'link_title' => esc_html__( 'Go to Customizer','dupermag' ),
        ...
        'third' => array(
	        'title' => esc_html__( 'Step 3 : Setting Featured Section ','dupermag' ),
	        'desc' => esc_html__( 'Add Feature Image or Unlimited Slides on Feature Section. Please do the Step 3 before this.','dupermag' ),
	        'link_title' => esc_html__( 'Featured Section','dupermag' ),
        ...
        'forth' => array(
	        'title' => esc_html__( 'Step 4 : Setting Home Main Content Area ','dupermag' ),
	        'desc' => esc_html__( 'Add different section on Home Main Content Area using Widgets','dupermag' ),
	        'link_title' => esc_html__( 'Add different section on Home Page','dupermag' ),
    ...
    'recommended_actions'        => array(
	    'setting-image' => array(
		    'title' => esc_html__( 'Setting Image Size','dupermag' ),
		    'desc' => sprintf( esc_html__( 'IMPORTANT!!! Set thumbnail size : 500 * 280, Set Medium size : 660 * 365, Set Large size : 840 * 480 %1$s Set Image Size %2$s ', 'dupermag' ), '<p><a href="#" class="acme-set-image button button-primary">','</a></p>' ),
		    'id' => 'setting-image'
	    ),
        'demo-content' => array(
            'title' => esc_html__( 'Install Demo Setup','dupermag' ),
            'desc' => sprintf( esc_html__( 'For the demo content installation, install the Acme Demo Setup plugin, download demo content from %1$s here %2$s. Then go to "Acme Demo Setup" and import demo. ', 'dupermag' ), '<a target="_blank" href="http://www.doc.acmethemes.com/dupermag/#DummyData" >','</a>' ),
            'id' => 'acme-demo-setup',
    ...
    'faq'      => array(
	    'first' => array (
		    'title' => esc_html__( 'Where to get a dedicated support ? ','dupermag' ),
		    'icon' => 'dashicons dashicons-sos',
		    'desc' => esc_html__( 'Follow the theme dedicated support forum.','dupermag' ),
		    'button_label' => esc_html__( 'Click here to get the support','dupermag' ),
	    ...
	    'second' => array (
		    'title' => esc_html__( 'Recent Updates of your Site ? ','dupermag' ),
		    'icon' => 'dashicons dashicons-sos',
		    'desc' => esc_html__( 'Where i can get the recent updates of theme related updates ? ','dupermag' ),
		    'button_label' => esc_html__( 'Visit our site blog page','dupermag' ),
		    'button_link' => esc_url( 'https://www.acmethemes.com/blog/' ),
		    'is_button' => true,
		    'is_new_tab' => true
	    ),
	    'third' => array (
		    'title' => esc_html__( 'How to make image size suitable for the theme ? ','dupermag' ),
		    'icon' => 'dashicons dashicons-sos',
		    'desc' => esc_html__( 'In this recent update, you can check Appearance > DuperMag Info > Recommended Action > Setting Image Size and click on Set Image size. Now your theme image size will be automatically updates on Settings > Media.','dupermag' )
	    ),

	    'forth' => array (
		    'title' => esc_html__( 'How to hide/change footer text Proudly powered by WordPress | Theme: DuperMag by AcmeThemes. ? ','dupermag' ),
		    'icon' => 'dashicons dashicons-sos',
		    'button_label' => esc_html__( 'Visit this blog post to know how to change or remove powered by text.','dupermag' ),
    ...
    'support_content'      => array(
        'first' => array (
            'title' => esc_html__( 'Contact Support','dupermag' ),
            'icon' => 'dashicons dashicons-sos',
            'desc' => esc_html__( 'Got theme support question or found bug? Best place to ask your query is our dedicated Support forum.','dupermag' ),
            'button_label' => esc_html__( 'Contact Support','dupermag' ),
            'button_link' => esc_url( 'https://www.acmethemes.com/supports/forum/dupermag/' ),
            'is_button' => true,
            'is_new_tab' => true
        ),
        'second' => array(
            'title' => esc_html__( 'Documentation','dupermag' ),
            'icon' => 'dashicons dashicons-book-alt',
            'desc' => esc_html__( 'Please check our full documentation for detailed information on how to Setup and Use DuperMag.','dupermag' ),
            'button_label' => esc_html__( 'Read full documentation','dupermag' ),
            'button_link' => 'http://www.doc.acmethemes.com/dupermag/',
            'is_button' => false,
            'is_new_tab' => true
        ),
        'third' => array(
            'title' => esc_html__( 'Need more features?','dupermag' ),
            'icon' => 'dashicons dashicons-screenoptions',
            'desc' => esc_html__( 'Upgrade to PRO version for more exciting features and Priority Support.','dupermag' ),
            'button_label' => esc_html__( 'View Pro Version','dupermag' ),
            'button_link' => 'https://www.acmethemes.com/themes/dupermagpro/',
            'is_button' => true,
            'is_new_tab' => true
        ),
        'fourth' => array(
            'title' => esc_html__( 'Got sales related question?','dupermag' ),
            'icon' => 'dashicons dashicons-cart',
            'desc' => esc_html__( "Have any query before purchase, you are more than welcome to ask.",'dupermag' ),
            'button_label' => esc_html__( 'Pre-sale Question?','dupermag' ),
            'button_link' => 'https://www.acmethemes.com/pre-sale-question/',
            'is_button' => true,
            'is_new_tab' => true
        ),
        'fifth' => array(
            'title' => esc_html__( 'Customization Request','dupermag' ),
            'icon' => 'dashicons dashicons-hammer',
            'desc' => esc_html__( 'Needed any customization for the theme, you can request from here.','dupermag' ),
            'button_label' => esc_html__( 'Customization Request','dupermag' ),
    ...
    'free_pro' => array(
	    array(
		    'title'=> __( 'Custom Widgets', 'dupermag' ),
		    'desc' => __( 'All added custom widgets are ready for page builder plugin', 'dupermag' ),
		    'free' => __('2+ Available','dupermag'),
		    'pro'  => __('12+ Available','dupermag'),
	    ),
	    array(
		    'title'       => __( 'Widget Options', 'dupermag' ),
		    'desc' => __( 'Every widgets have multiple options to customize section', 'dupermag' ),
		    'free'  => __('Limited','dupermag'),
		    'pro'   => __('Multiple Options','dupermag'),
	    ),
	    array(
		    'title'       => __( 'Google Fonts', 'dupermag' ),
		    'desc' => __( 'Google fonts options for changing the overall site fonts ', 'dupermag' ),
		    'free'  => 'yes',
		    'pro'   => __('100 +','dupermag'),
	    ),
	    array(
		    'title'       => __( 'Color Options', 'dupermag' ),
		    'desc' => __( 'Color options for changing overall site colors, Contat us', 'dupermag' ),
		    'free'  => __('2','dupermag'),
		    'pro'   => __('Unlimited','dupermag'),
	    ),
	    array(
		    'title'       => __( 'Header Options', 'dupermag' ),
		    'desc' => __( 'Customze the header section as your need.', 'dupermag' ),
		    'free'  => __('Limited','dupermag'),
		    'pro'   => __('Multiple Options','dupermag'),
	    ),
	    array(
		    'title'       => __( 'Breaking News Options', 'dupermag' ),
		    'desc' => __( 'Manage your breaking news section on header', 'dupermag' ),
		    'free'  => __('Limited','dupermag'),
		    'pro'   => __('Multiple Options','dupermag'),
	    ),
	    array(
		    'title'       => __( 'Menu Options', 'dupermag' ),
		    'desc' => __( 'Lots of menu options are available to manage the menu section. Appearance > Menus', 'dupermag' ),
		    'free'  => __('Limited','dupermag'),
		    'pro'   => __('Multiple Options','dupermag'),
	    ),
	    array(
		    'title'       => __( 'Featured Selection', 'dupermag' ),
		    'desc' => __( 'This is slider section. Check every options there','dupermag' ),
		    'free'  => __('Parent/Child Page','dupermag'),
		    'pro'   => __('Page, Post, Category, And Custom','dupermag'),
	    ),
	    array(
		    'title'       => __( 'Featured Section Options', 'dupermag' ),
		    'desc' => __( 'Options to manage the sliders', 'dupermag' ),
		    'free'  => __('Limited','dupermag'),
		    'pro'   => __('Multiple Options','dupermag'),
	    ),
	    array(
		    'title'       => __( 'Sticky Sidebar', 'dupermag' ),
		    'desc' => __( 'Enable/Disable sticky sidebars', 'dupermag' ),
		    'free'  => 'no',
		    'pro'   => 'yes',
	    ),
	    array(
		    'title'       => __( 'Blog/Archive Options', 'dupermag' ),
		    'desc' => __( 'Various Options are available for the blog and archive page management. Check on Appearance > Customize > Layout/Design Options.', 'dupermag' ),
		    'free'  => __('Limited','dupermag'),
		    'pro'   => __('Manage Every Entity','dupermag'),
	    ),
	    array(
		    'title'       => __( 'Footer Options', 'dupermag' ),
		    'desc' => __( 'Manage the every entity related to footer section.', 'dupermag' ),
		    'free'  => __('Limited','dupermag'),
		    'pro'   => __('Multiple Options','dupermag'),
	    ),
	    array(
		    'title'       => __( 'Social Options', 'dupermag' ),
		    'desc' => __( 'Options to manage the social icons like facebook, instagram, linkedin etc.', 'dupermag' ),
		    'free'  => __('Limited','dupermag'),
		    'pro'   => __('Multiple Options','dupermag'),
	    ),
	    array(
		    'title'       => __( 'Theme Credit Link', 'dupermag' ),
		    'desc' => __( 'Options to disable to Powered by text on footer.', 'dupermag' ),
		    'free'  => __('No Options To Disable','dupermag'),
		    'pro'   => __('Enable/Disable','dupermag'),
	    ),
	    array(
		    'title'       => __( 'Own Credit Link', 'dupermag' ),
		    'desc' => __( 'Write your own credit text.', 'dupermag' ),
		    'free'  => 'no',
		    'pro'   => __('Enable/Disable','dupermag'),
	    ),
	    array(
		    'title'       => __( 'Social Icons', 'dupermag' ),
		    'desc' => __( 'Enter the URL for social icons. ', 'dupermag' ),
		    'free'  => __('5 +','dupermag'),
		    'pro'   => __('Available Mostly Used','dupermag'),
	    ),
	    array(
		    'title'       => __( 'Intro Loader Options', 'dupermag' ),
		    'desc' => __( 'It will load before the site loaded completely.', 'dupermag' ),
		    'free'  => 'no',
		    'pro'   => __('Multiple Options','dupermag'),
	    ),
	    array(
		    'title'       => __( 'Related Post Options', 'dupermag' ),
		    'desc' => __( 'Related post will appear on single post of each blog article', 'dupermag' ),
		    'free'   => __('Enable/Disable','dupermag'),
		    'pro'   => __('Multiple Options','dupermag'),
	    ),
	    array(
		    'title'       => __( 'Author Info Options', 'dupermag' ),
		    'desc' => __( 'This will appear on single page and archive page of that author.', 'dupermag' ),
		    'free'   => __('Default','dupermag'),
		    'pro'   => __('Multiple Options','dupermag'),
	    ),
	    array(
		    'title'       => __( 'Feature Image Options', 'dupermag' ),
		    'desc' => __( 'Featured image options for the single page and post.', 'dupermag' ),
		    'free'  => 'no',
		    'pro'   => __('Manage As Your Need','dupermag'),
	    ),
	    array(
		    'title'       => __( 'Navigation Options', 'dupermag' ),
		    'desc' => __( 'Post Navigation means, previous post and next post. Check on single post.', 'dupermag' ),
		    'free'   => __('Default','dupermag'),
		    'pro'   => __('Advanced','dupermag'),
	    ),
	    array(
		    'title'       => __( 'Post Comment Options', 'dupermag' ),
		    'desc' => __( 'Manage the comment section on single post.', 'dupermag' ),
		    'free'  => 'no',
		    'pro'   => __('Options Available','dupermag'),
	    ),
	    array(
		    'title'       => __( 'Show/Hide Comment', 'dupermag' ),
		    'desc' => __( 'Options to show hide comments on page and post.', 'dupermag' ),
		    'free'  => 'no',
		    'pro'   => 'yes',
	    ),
	    array(
		    'title'       => __( 'Breadcrumb', 'dupermag' ),
		    'desc' => __( 'Advanced options for manage the Breadcrumb.', 'dupermag' ),
		    'free'  => 'yes',
		    'pro'   => __('Advanced','dupermag'),
	    ),
	    array(
		    'title'       => __( 'Video Post On Featured', 'dupermag' ),
		    'desc' => __( 'Create your video post easily from Youtube and Vimeo video.', 'dupermag' ),
		    'free'  => 'no',
		    'pro'   => __('Add Own Video On Featured','dupermag'),
	    ),
	    array(
		    'title'=> __( 'Single Post Options', 'dupermag' ),
		    'desc' => __( 'Manage every entity of single post.', 'dupermag' ),
		    'free' => __('Limited','dupermag'),
		    'pro'  => __('Multiple Options','dupermag'),
	    ),
	    array(
		    'title'=> __( 'Featured Image Size', 'dupermag' ),
		    'desc' => __( 'Select the size of the featured image on the single post.', 'dupermag' ),
		    'free' => 'no',
		    'pro'  => 'yes',
	    ),
	    array(
		    'title'=> __( 'Sidebars', 'dupermag' ),
		    'desc' => __( 'Global and Individual sidebars are available. You can easily orveride it from single page/post.', 'dupermag' ),
		    'free' => 'Yes',
		    'pro'  => __('Global And Individuals', 'dupermag'),
	    ),
	    array(
		    'title'=> __( 'WooCommerce', 'dupermag' ),
		    'desc' => __( 'Create WooCommerce shop easily with the help of WooCommerce plugin.', 'dupermag' ),
		    'free' => 'Yes',
		    'pro'  => 'yes',
	    ),
	    array(
		    'title'       => __( 'Translation', 'dupermag' ),
		    'desc' => __( 'Both theme are translation ready.', 'dupermag' ),
		    'free'  => 'yes',
		    'pro'   => 'yes',
	    ),
	    array(
		    'title'       => __( 'SEO', 'dupermag' ),
		    'desc' => __( 'Developed with high skilled SEO tools.', 'dupermag' ),
		    'free'  => 'yes',
		    'pro'   => 'yes',
	    ),
	    array(
		    'title'       => __( 'Widget Area On Footer', 'dupermag' ),
		    'desc' => __( 'Change the number of footers on footer area as you need.', 'dupermag' ),
		    'free'  => __('3 ', 'dupermag'),
		    'pro'   => __('Choose On Need', 'dupermag'),
	    ),
	    array(
		    'title'       => __( 'Support Forum', 'dupermag' ),
		    'desc' => __( 'Highly dedicated support team are assigned for your help. Try this today.', 'dupermag' ),
		    'free'  => __('Second', 'dupermag'),
		    'pro'   => __('Dedicated With High Priority', 'dupermag'),
	    )
